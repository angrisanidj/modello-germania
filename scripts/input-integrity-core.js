(function(root,factory){
  ...
  function evaluateIntegrity(args={}){
    ...
    const knownMismatch=missingUpstream.length>0||missingModel.length>0||unexpectedDataset.length>0,sourceVerified=source.status==='verified'&&Number.isFinite(sourceAgeHours)&&sourceAgeHours<=staleAfter,acquisitionFresh=Number.isFinite(acquisitionAgeHours)&&acquisitionAgeHours<=staleAfter;
    let state='green',reason='Fonte, dataset e motore coerenti.';if(knownMismatch){state='red';reason='Le rilevazioni eleggibili divergono lungo la catena dati.'}else if(!sourceVerified||!acquisitionFresh){state='yellow';reason='Completezza o freschezza upstream non certificata.'}
    return{state,reason,sourceVerified,acquisitionFresh,sourceMode:source.mode||'unknown',sourceStatus:source.status||'unknown',sourceAgeHours:Number.isFinite(sourceAgeHours)?sourceAgeHours:null,acquisitionAgeHours:Number.isFinite(acquisitionAgeHours)?acquisitionAgeHours:null,counts:{expected:expected.size,dataset:dataset.size,used:used.size},missingUpstream,missingModel,unexpectedDataset};
  }
